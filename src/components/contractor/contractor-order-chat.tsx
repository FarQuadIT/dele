...
import { contractorSendOrderMessage } from "@/lib/actions/contractor-actions";
...
export function ContractorOrderChat({
  orderId,
  currentUserId,
  ...
  async function send() {
    ...
    const result = await contractorSendOrderMessage({ orderId, body: text });
    setSending(false);
    if (!result.ok) {
